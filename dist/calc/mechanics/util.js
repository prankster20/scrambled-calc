...
function getFinalSpeed(gen, pokemon, field, side) {
    ...
    else if (isQPActive(pokemon, field) &&
...
function checkWonderRoom(pokemon, wonderRoomActive) {
    ...
    if (wonderRoomActive) {
        ...
        ], 2), pokemon.rawStats.def = _a[0], pokemon.rawStats.spd = _a[1];
    }
}
...
function checkInstinct(source, target, wonderRoomActive) {
    var _a;
...
exports.checkInstinct = checkInstinct;
...
function checkMultihitBoost(gen, attacker, defender, move, field, desc, attackerUsedItem, defenderUsedItem) {
    ...
    if (move.named("Gyro Ball", "Electro Ball") &&
    ...
    if (move.dropsStats) {
        if (attacker.hasAbility("Unaware")) {
            ...
            if (atkSimple === 2)
                desc.attackerAbility = attacker.ability;
            if (attacker.hasItem("White Herb") &&
    ...
    if (defender.hasAbility("Mummy", "Hypnotize", "Lingering Aroma") &&
        move.flags.contact) {
...
function getBaseDamage(level, basePower, attack, defense) {
    return Math.floor(OF32(Math.floor(OF32(OF32(Math.floor((2 * level) / 5 + 2) * basePower) * attack) /
        defense) /
        50 +
        2));
}
...
function getWeight(pokemon, desc, role) {
    ...
    var abilityFactor = pokemon.hasAbility("Heavy Metal")
        ? 2
